...
class SmartPool:
    ...
    def get_result(self) -> Union[JobResult, None]:
        try:
    ...
    def __call__(self):
        while True:
            ...
            for p in died:
                # might have died after calling get get_result() which clears the current_job to None
                # would also mean we don't want to send back this error message since we already returned a result for this job
                if p.current_job is not None:
            ...
            # ignore processes that already returned a result
            for p in [p for p in overtime if p not in results_ready]:
                p.terminate()
            ...
            # ignore processes that already returned a result
            for p in [p for p in oom if p not in results_ready]:
                p.terminate()
